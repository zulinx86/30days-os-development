			...
			if (256 <= i && i < 512) {
				if (i == 8 + 256) { /* backspace */
					...
					if (mystrcmp(cmdline, "mem") == 0) {
						...
						cursor_y = cons_newline(cursor_y, sheet);
						mysprintf(s, "free: %d KB", memman_total(memman) / 1024);
						putfonts8_asc_sht(sheet, 8, cursor_y, COL8_FFFFFF, COL8_000000, s, 30);
						cursor_y = cons_newline(cursor_y, sheet);
						cursor_y = cons_newline(cursor_y, sheet);
						...
						for (x = 0; x < 224; x++) {
							...
							if (finfo[x].name[0] != 0xe5) {
								if ((finfo[x].type & 0x18) == 0) {
									mysprintf(s, "filename.ext   %7d", finfo[x].size);
									for (y = 0; y < 8; y++)
										s[y] = finfo[x].name[y];
									s[9] = finfo[x].ext[0];
									s[10] = finfo[x].ext[1];
						...
						if (x < 224 && finfo[x].name[0] != 0x00) {
							...
							file_loadfile(finfo[x].clustno, finfo[x].size, p, fat, (char *) (ADR_DISKIMG + 0x00003e00));
							cursor_x = 8;
