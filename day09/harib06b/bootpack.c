...
unsigned int memtest_sub(unsigned int start, unsigned int end)
{
	...
	for (i = start; i <= end; i += 0x1000) {
		...
		old = *p;			/* old value */
		*p = pat0;			/* write pattern 0 */
		*p ^= 0xffffffff;	/* inverse */
		if (*p != pat1) {	/* compare with pattern 1 */
not_memory:
			*p = old;
			break;
		}
		*p ^= 0xffffffff;	/* inverse again */
		if (*p != pat0) {	/* compare with pattern 0 */
			goto not_memory;
		}
		*p = old;			/* set old value back */
	}
	return i;
}
