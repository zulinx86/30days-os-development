...
struct SHTCTL *shtctl_init(struct MEMMAN *memman, char *vram, int xsize, int ysize)
{
	...
	if (ctl->map == 0) {
		memman_free_4k(memman, (int) ctl, sizeof(struct SHTL));
		goto err;
	}
...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	if (old > height) { /* the new height is lower than the old one */
		if (height >= 0) {
	...
	} else if (old < height) { /* the new height is higher than the old one */
		if (old >= 0) {
			for (h = old; h < height; h++) {
				ctl->sheets[h] = ctl->sheets[h + 1];
				ctl->sheets[h]->height = h;
		...
		sheet_refreshmap(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vx0 + sht->bysize, height);
