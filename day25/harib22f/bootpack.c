...
void HariMain(void)
{
	...
	struct SHEET *sht, *sht_back, *sht_mouse, *sht_win, *sht_cons[2];
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i < 512) {
				...
				if (key_ctrl && s[0] == 'C' && task_cons[0]->tss.ss0 != 0) {
				...
				if (i == 256 + 0x80)
					fifo32_put(&key_win->task->fifo, 0x80 + 256);

				if (key_ctrl && i == 256 + 0x0f) { /* tab */
					if (shtctl->top > 2) {
