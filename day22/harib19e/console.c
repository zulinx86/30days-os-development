...
void cons_putnstr(struct CONSOLE *cons, char *s, int l)
{
	int i;
	for (i = 0; i < l; i++)
...
{
	if (mystrcmp(cmdline, "mem") == 0) cmd_mem(cons, memtotal);
	else if (mystrcmp(cmdline, "cls") == 0) cmd_cls(cons);
	else if (mystrcmp(cmdline, "dir") == 0) cmd_dir(cons);
	else if (mystrncmp(cmdline, "type ", 5) == 0) cmd_type(cons, fat, cmdline);
	else if (cmdline[0] != 0) {
...
{
	struct MEMMAN *memman = (struct MEMMAN *) MEMMAN_ADDR;
	struct FILEINFO *finfo = file_search(cmdline + 5, (struct FILEINFO *) (ADR_DISKIMG + 0x00002600), 224);
	char *p;

	if (finfo != 0) {
		p = (char *) memman_alloc_4k(memman, finfo->size);
		file_loadfile(finfo->clustno, finfo->size, p, fat, (char *) (ADR_DISKIMG + 0x00003e00));
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	else if (edx == 2) cons_putstr(cons, (char *) ebx + cs_base);
	else if (edx == 3) cons_putnstr(cons, (char *) ebx + cs_base, ecx);
