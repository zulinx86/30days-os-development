...
void putstr(int win, unsigned char *winbuf, int x, int y, int col, unsigned char *s);
void wait(int i, int timer, unsigned char *keyflag);
...
void HariMain(void)
{
	...
	unsigned char winbuf[336 * 261], invstr[32 * 6], s[12], keyflag[4], *p;
	static unsigned char invstr0[32] = " abcd abcd abcd abcd abcd ";

	win = api_openwin(winbuf, 336, 261, -1, (unsigned char *) "invader");
	api_boxfillwin(win, 6, 27, 329, 254, 0);
	timer = api_alloctimer();
	api_inittimer(timer, 128);

	high = 0;
	putstr(win, winbuf, 22, 0, 7, (unsigned char *) "HIGH:00000000");

restart:
	score = 0;
	point = 1;
	putstr(win, winbuf,  4, 0, 7, (unsigned char *) "SCORE:00000000");
	movewait0 = 20;
	fx = 18;
	putstr(win, winbuf, fx, 13, 6, (unsigned char *) "efg");
	...
	for (;;) {
		...
		if (movewait != 0) {
			...
			if (ix + idir > 14 || ix + idir < 0) {
				...
				idir = - idir;
				putstr(win, winbuf, ix + 1, iy, 0, (unsigned char *) "                         ");
				iy++;
			} else {
				ix += idir;
			}
		...
		if (ly > 0) {
			if (ly < 13) {
				if (ix < lx && lx < ix + 25 && iy <= ly && ly < iy + invline) {
					...
					putstr(win, winbuf, lx, ly, 0, (unsigned char *) " ");
				}
			}
			ly--;
			if (ly > 0) {
				putstr(win, winbuf, lx, ly, 3, (unsigned char *) "h");
			} else {
				point -= 10;
				if (point <= 0) {
					point = 1;
			...
			if (ix < lx && lx < ix + 25 && iy <= ly && ly < iy + invline) {
				...
				if (*p != ' ') {
					...
					score += point;
					point++;
					mysprintf(s, (unsigned char *) "%08d", score);
