...
/* 10進数からASCIIコードに変換 */
int dec2asc(unsigned char *str, int dec, int len, int zero) {
...
/* 16進数からASCIIコードに変換 */
int hex2asc(unsigned char *str, int dec, int len, int zero, int upper) {
	int len_buf = 0, tmp;
	int buf[10];
...
void mysprintf(unsigned char *str, unsigned char *fmt, ...) {
	va_list args;
	int len, zero = 0;
