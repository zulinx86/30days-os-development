...
void HariMain(void)
{
	...
	int i, j, x, y, mx, my, new_mx = -1, new_my = 0, new_wx = 0x7fffffff, new_wy = 0, mmx = -1, mmy = -1, mmx2 = 0;
	struct TASK *task, *task_kernel;
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	finfo = file_search("nihongo.fnt", (struct FILEINFO *) (ADR_DISKIMG + 0x00002600), 224);
	if (finfo != 0) {
		file_loadfile(finfo->clustno, finfo->size, nihongo, fat, (unsigned char *) (ADR_DISKIMG + 0x00003e00));
	} else {
		for (i = 0; i < 16 * 256; ++i)
			nihongo[i] = hankaku[i];
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i < 512) {
				...
				if (key_ctrl && key_win != 0) {
					if (s[0] == 'C') {
						task = key_win->task;
						if (task != 0 && task->tss.ss0 != 0) {	
							cons_putstr(task->cons, (unsigned char *)"\nBreak(key) :\n");
							io_cli();
				...
				if (key_ctrl && i == 256 + 0x0f && key_win != 0) { /* ctrl + tab */
					keywin_off(key_win);
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	make_window8(buf, 256, 165, (unsigned char *)"console", 0);
